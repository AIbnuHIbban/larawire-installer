...
namespace Larawire\Installer\Console;
...
class NewCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        if (version_compare(PHP_VERSION, '7.3.0', '<')) {
            throw new RuntimeException('The Larawire installer requires PHP 7.3.0 or greater. Please use "composer create-project aibnuhibban/larawire" instead.');
        }
        ...
        $output->writeln('<info>Membuat Aplikasi...</info>');
        ...
        if ($process->isSuccessful()) {
            $output->writeln('<comment>Aplikasi Selesai! Silahkan buat aplikasi keren anda.</comment>');
        }
    ...
    protected function verifyApplicationDoesntExist($directory)
    {
        if ((is_dir($directory) || is_file($directory)) && $directory != getcwd()) {
            throw new RuntimeException('Aplikasi sudah ada!');
        }
    }
